...
<html lang="es">
    ...
    <body>
        ...
        <?php
            // Simula la conexión a la base de datos (ajusta según tu configuración)
            ...
            if ($idUsuario) {
                ...
                $query = "SELECT c.idCompania, c.nombreCompania, COUNT(dp.idJuego) as totalJuegosVendidos FROM compran co
                        JOIN detalle_pedido dp ON co.idPed = dp.idPed JOIN juegos j ON dp.idJuego = j.idJuego JOIN compania c ON j.idCompania = c.idCompania
                        WHERE co.idUsuario = '$idUsuario' GROUP BY c.idCompania ORDER BY totalJuegosVendidos DESC LIMIT 1";
                ...
                if ($resultado_compras) {
                    if ($resultado_compras->num_rows > 0) {
                        ...
                        $queryRecomendacion = "SELECT nombre, plataforma, precio FROM juegos  WHERE idCompania = $idCompaniaRecomendada ORDER BY RAND() LIMIT 3";
                        ...
                        if ($resultadoRecomendacion->num_rows > 0) {
                            ...
                            $recomendacion = "Juegos de la compañía que más has comprado:<br>";
                            while ($filaRecomendada = $resultadoRecomendacion->fetch_assoc()) {
                                $recomendacion .= "Nombre: " . $filaRecomendada['nombre'] . "<br>";
                                $recomendacion .= "Plataforma: " . $filaRecomendada['plataforma'] . "<br>";
                                $recomendacion .= "Precio: €" . $filaRecomendada['precio'] . "<br>";
                                $recomendacion .= "-------------------------<br>";
                            }
                        } else 
                            $recomendacion = "No hay juegos de la compañía recomendada en este momento."; 
                    } else {
                        $queryJuegoMasStockVendido = "  SELECT j.idJuego, j.nombre, j.plataforma, j.precio, j.stock - COALESCE(SUM(dp.cantidad), 0) as stockVendido
                                                        FROM juegos j LEFT JOIN detalle_pedido dp ON j.idJuego = dp.idJuego GROUP BY j.idJuego ORDER BY stockVendido DESC LIMIT 1";
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                document.getElementById('recomendaciones').innerHTML = 'Juegos recomendados: ' + recomendaciones;
            });
        </script>
    </body>
</html>